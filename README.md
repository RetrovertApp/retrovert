...
(`<channel>/root.json`), the [`retrovert-publish`](crates/retrovert-publish)
CLI that signs it, and the scheduled job that keeps its metadata from expiring.
Clients use [`retrovert-updater`](https://github.com/RetrovertApp/retrovert-updater);
this repository pins `retrovert-tuf` from it so signer and verifier agree on the
metadata format.
...
| --- | --- | --- | --- |
| `dev` | `RetrovertApp/playback_plugins` | `https://github.com/RetrovertApp/playback_plugins/releases/download/dev/channel-metadata/` | [`dev/root.json`](dev/root.json) |
